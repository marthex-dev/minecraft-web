...
$(function () {
    ...
    $('[data-toggle="imageInput"]').change(function() {
        readImage(this);
    });
...
function readImageTheme(input, image, imageInput) 
{
  if (input.files && input.files[0]) {
      var reader = new FileReader();
...
function selectAnswer(select, target, target2 = false) 
{
...
function serverCheck(form, button)
{
  
  $(button).html('Bağlantı Kontrol Ediliyor.');

  $.ajax({
      type: "POST",
      url: "classes/ajax/Sender.test.php",
      data : $(form).serialize(),
      success: function(result) {
        if (result == true) {
          $(button).html('<span class="text-success">Konsol bağlantısı başarılı.</span> (Tekrar kontrol et)');
        
        } else {
          $(button).html('<span class="text-danger">Konsol bağlantısı başarısız.</span> (Tekrar kontrol et)');
        }
      }
...
function getServerCategories(selectID, responseID, categoryID = 0, type = 0)
{
  
  var id = $(selectID).val();

  $(responseID).html('<option>Yükleniyor...</option>');

  if (categoryID != 0) {

    $.ajax({
        type: "POST",
        url: "classes/ajax/Store.categories.php",
        data : {id : id, categoryID : categoryID, type : type},
