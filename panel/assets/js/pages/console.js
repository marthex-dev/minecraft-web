$(document).ready(function(){
  ...
  $("#txtCommand").bind("enterKey",function(e){
      var server = $("#txtCommand").attr('server-id');
      sendCommand($("#txtCommand").val(), server);
      $("#txtCommand").val('Lütfen Bekleyiniz');
      $("#txtCommand").attr('disabled', 'disabled');
  });
...
function sendCommand(command, serverID){
  if (command == "") {
    alertDanger("Command missing.");
    return;
  }
  logMsg(command, ">", "success");
  $.post("classes/ajax/Console.ajax.php", { cmd: command, server: serverID })
    .done(function(json){
      if(json.status){
        if(json.status == 'success' && json.response && json.command){
          if(json.response.indexOf("Komut Bulunamadı") != -1){
            alertDanger("Komut Bulunamadı : " + json.command); 
          ...
          alertDanger("Hata! Sunucudan cevap alınamadı.1"); 
          logDanger("Hata! Sunucudan cevap alınamadı.");
            $("#txtCommand").removeAttr('disabled');
            $("#txtCommand").val("");
        }
      }
      else{
        alertDanger("Hata! Sunucudan cevap alınamadı.2"); 
        logDanger("Hata! Sunucudan cevap alınamadı.");
        $("#txtCommand").removeAttr('disabled');
        $("#txtCommand").val("");
      }
    ...
    .fail(function() {
      alertDanger("Hata! Sunucuyla bağlantı kurulamadı.");
      logDanger("Hata! Sunucuyla bağlantı kurulamadı.");
      $("#txtCommand").removeAttr('disabled');
      $("#txtCommand").val("");
    });
}